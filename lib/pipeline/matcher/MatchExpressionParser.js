		...
		if (currname[0] == '$' ) {
			...
			if ("or" == rest) {
				...
				*/
				...
				return {code:ErrorCodes.BadValue, description:"unknown top level operator: " + currname};
			}
...
proto._parseSubField = function _parseSubField(context, andSoFar, name, element){
	...
	if ("$eq" == currname)
		return this._parseComparison(name, new EqualityMatchExpression(), currval);
	...
	switch (currname) {
	case '$lt':
		return this._parseComparison(name, new LTMatchExpression(), currval);
	case '$lte':
		return this._parseComparison(name, new LTEMatchExpression(), currval);
	case '$gt':
		return this._parseComparison(name, new GTMatchExpression(), currval);
	case '$gte':
		return this._parseComparison(name, new GTEMatchExpression(), currval);
	case '$ne':
		status = this._parseComparison(name, new EqualityMatchExpression(), currval);
		if (status.code != ErrorCodes.OK)
			return status;
		var n = new NotMatchExpression();
		status = n.init(status.result);
